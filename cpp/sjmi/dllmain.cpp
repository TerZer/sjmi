...
(JNIEnv* env, jobject thisObject, jlong videoProp, jlong value, jlong flags)
{
    ...
    hr = pProcAmp->Set(videoProp, value, flags); // set new value
    if (SUCCEEDED(hr))
    {
...
(JNIEnv* env, jobject thisObject, jlong cameraProp, jlong value, jlong flags)
{
...
// https://docs.microsoft.com/en-us/windows/win32/api/strmif/ne-strmif-cameracontrolproperty
    ...
    hr = pCamControl->Set(cameraProp, value, flags); // set new value; exposure: For values less than zero, the exposure time is 1/(2n) seconds. For values of zero and above, the exposure time is 2n seconds.
    if (SUCCEEDED(hr))
    {
...
(JNIEnv* env, jobject thisObject, jint returnFormat, jobject rcvr)
{
    ...
    if (activeDeviceCount > 0)
    {
        ...
        // Ugly switch equiv.
        if (GetGUIDNameConst(videoFormatGUID) != NULL) {
        ...
        if (inputFormat == FORMAT_NONE) inputStreamLen = (frameWidth * frameHeight);
        ...
        if (returnFormat == 3) outputStreamLen = (frameWidth * frameHeight);
